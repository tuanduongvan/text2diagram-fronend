...
export const computeEdgePositionsMC = (
  pathElement: SVGPathElement,
  offset: Position = { x: 0, y: 0 }
): EdgePositionData => {
  if (pathElement.tagName.toLowerCase() !== 'path') {
  ...
  // Extract only M and C commands
  const commands = dAttr.match(/[MC][^MC]*/g) || [];

  if (commands.length === 0 || !commands[0]?.startsWith('M')) {
    throw new Error("Path must start with an 'M' command.");
  }

  // Extract start position (M command)
  const startPosition = commands[0]
    .substring(1)
    .trim()
    .split(/[\s,]+/)
  ...
  // Extract last position from the last C command
  const lastCommand = commands[commands.length - 1];
  if (!lastCommand.startsWith('C')) {
    throw new Error("No valid 'C' (Cubic Bezier Curve) command found.");
  }

  const lastCoords = lastCommand
    .substring(1)
    .trim()
    .split(/[\s,]+/)
    .map((coord) => parseFloat(coord));

  if (lastCoords.length < 6) {
    throw new Error("Invalid 'C' command. Expected 6 coordinate values.");
  }

  // Extract end position (last control point of last C command)
  const endPosition = lastCoords.slice(-2);

  // Extract reflection points (Bezier control points + end points)
  const reflectionPoints = commands
    .filter((cmd) => cmd.startsWith('C')) // Only process C commands
    .flatMap((command) => {
      const coords = command
        .substring(1)
        .trim()
